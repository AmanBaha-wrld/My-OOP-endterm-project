...
public class ClientOrder implements ClientRepositories {
    ...
    @Override
    public List<Client>  getTime(String title, Time s_time, Time f_time) {
        Connection con = null;
        try {
            con = db.getConnection();
            String sql = "SELECT f_session.session_id, film.title, cinema.title, cinema.addres, f_session.s_time, f_session.f_time, f_session.price, f_session.hall_id\n" +
                    "FROM cinema \n" +
                    "JOIN f_session\n" +
                    "ON f_session.s_time>=? AND f_session.s_time<=?\n" +
                    "JOIN film\n" +
                    "ON film.title=?";
            PreparedStatement st = con.prepareStatement(sql);
            st.setTime(1,s_time);
    ...
    public String selectSession(int ID) {
        ...
        try {
            con = db.getConnection();
            String sql = "\n" +
                    "SELECT f_session.session_id, film.title, cinema.title, cinema.addres, f_session.s_time, f_session.f_time, f_session.price, f_session.hall_id\n" +
                    "FROM cinema\n" +
                    "JOIN f_session\n" +
                    "ON f_session.session_id=? \n" +
                    "JOIN film\n" +
                    "ON film.film_id=f_session.film_id;";
            PreparedStatement st = con.prepareStatement(sql);
            st.setInt(1,ID);
